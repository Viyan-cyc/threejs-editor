...
export class PowerStationBuilder implements ObjectBuilder {
  ...
  async build(_node: SceneObjectDSL, _ctx: EditorContext): Promise<THREE.Object3D> {
    ...
    // 混凝土底座
    const base = box({ w: 4, h: 0.3, d: 3, color: 0xb8b2a4, roughness: 0.9, metalness: 0, name: 'base' });
    group.add(base);

    // 变压器 + 瓷绝缘子 ×2（左右各一组）
    for (const [i, x] of [-1.1, 1.1].entries()) {
      const tank = cylinder({
        radiusTop: 0.5,
        radiusBottom: 0.5,
        height: 1.2,
    ...
    // 控制柜（前方）
    const cabinet = box({ w: 1.2, h: 1.8, d: 0.8, color: 0x4a5560, roughness: 0.5, metalness: 0.4, name: 'cabinet' });
    ...
    const tip = cone({ radius: 0.08, height: 0.25, color: 0x2a3038, segments: 6, metalness: 0.6, name: 'lightning.tip' });
