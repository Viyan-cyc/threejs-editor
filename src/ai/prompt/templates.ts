...
- 不确定具体参数时给出合理默认值。
- notes 是给用户看的「场景说明」，**只描述场景里有什么、怎么布置**（对象、布局、环境、光照），例如「南侧布置光伏阵列，旁设升压站；草地中央放一只橘猫；日间自然光照」。
- **notes 里不得出现任何生成方式或技术细节**——不要写「高精度模型」「混元」「模型生成」「采用…生成」「归一化」「约 X 米」等。原因：清单外对象是用混元还是简化几何、最终是否成功，都由引擎在构建场景时决定，可能失败并降级；你产出 DSL 时无法预知，写了就会与实际矛盾、误导用户。
